...
*/
    ...
    gameBoard[TELEPORT_X][TELEPORT_Y] = TELEPORT; // teleport player to a new location
    gameBoard[SHIELD_X][SHIELD_Y] = SHIELD_ITEM; //create shield
...
*/
            ...
            && !captured(player,monster) && !winner(player, WINNER_X, WINNER_Y))
	{
        ...
        if(getShield(player, gameBoard))
            {
                player.change_char(PLAYER_SHIELDED_CHAR);
            }
  	}
