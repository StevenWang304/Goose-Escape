...
extern Console out;
const int RANDOM_NUM_RANGE = 3; // chance of goose jumping over a wall
...
// if the player has a shield when captured, the goose respawn to its orginal position and the player does not lose the game.
    if(captured)
    {
        if(player.isShield())
        {
...
// generate a shield to the player
